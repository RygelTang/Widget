package rygel.cn.wheelselector;
 ...
 * @version 1.1.0
 */
public interface IWheelSelector {

    /**
     * 获取滚轮选择器可见数据项的数量
     * <p>
     * Get the count of current visible items in WheelSelector
     *
     ...
     * 默认情况下滚轮选择器可见数据项数量为7
     * <p>
     * Set the count of current visible items in WheelSelector
     * The count of current visible items in WheelSelector must greater than 1
     * Notice:count of current visible items in WheelSelector will always is an odd number, even you
     * can set an even number for it, it will be change to an odd number eventually
     * By default, the count of current visible items in WheelSelector is 7
     *
     ...
     * 滚轮选择器数据项是否为循环状态
     * <p>
     * Whether WheelSelector is cyclic or not
     *
     * @return 是否为循环状态
     */
    boolean isCyclic();
     ...
     * <p>
     * Set whether WheelSelector is cyclic or not
     * WheelSelector's items will be end to end and in an infinite loop if setCyclic true, and there
     * is no border whit scroll when WheelSelector in cyclic state
     *
     ...
     * 设置滚轮Item选中监听器
     *
     * @param listener 滚轮Item选中监听器{@link WheelSelector.OnItemSelectedListener}
     */
    void setOnItemSelectedListener(WheelSelector.OnItemSelectedListener listener);
     ...
     * {@link rygel.cn.dateselector.selector.WheelSelector.OnItemSelectedListener}回调监听或调用
     * {@link #getCurrentItemPosition()}
     * <p>
     * Get the position of current selected item in data source
     * Notice:The value by return will not change when WheelSelector scroll, this method will always
     ...
     * through {@link rygel.cn.dateselector.selector.WheelSelector.OnItemSelectedListener} or call
     ...
     * 前所根据这些参数改变的一些属性，比如
     * {@link rygel.cn.dateselector.selector.WheelSelector.OnWheelChangeListener}和
     * {@link rygel.cn.dateselector.selector.WheelSelector.OnItemSelectedListener}监听器中方法参数的值，因
     * 此你总该在调用该方法后考虑到相关影响
     ...
     * Call this method and set a new value may be reinitialize the location of WheelSelector. For
     * example, you call this method after scroll the WheelSelector and set selected item position
     * with a new value, WheelSelector will clear the related parameters last scroll set and reset
     * series of data, and make the position 3 as a new starting point of WheelSelector, this behavior
     * maybe influenced some attribute you set last time, such as parameters of method in
     * {@link rygel.cn.dateselector.selector.WheelSelector.OnWheelChangeListener} and
     * {@link rygel.cn.dateselector.selector.WheelSelector.OnItemSelectedListener}, so you must always
     ...
     * change by WheelSelector scrolled
     *
     * @return 当前被选中的数据项所显示的数据在数据源中的位置
     */
     ...
     * 获取数据列表
     * <p>
     * Get data source of WheelSelector
     *
     * @return 数据列表
     */
    List getData();
     ...
     * {@link #setSelectedItemPosition(int)}
     * <p>
     * Set data source of WheelSelector
     * The data source can be any type, WheelSelector will change the data to string when it draw the
     * item.
     * There is a default data source when you not set the data source for WheelSelector.
     * Set data source for WheelSelector will reset state of it, you can refer to
     ...
     * 置，但是知道最宽的数据文本，那么你也可以直接通过调用setMaximumWidthText告诉滚轮选
     * 择器最宽的文本是什么，滚轮选择器会根据这条文本计算宽度并将其作为滚轮选择器的宽度
     * <p>
     * Set items of WheelSelector if has same width
     * WheelSelector will traverse the data source to calculate each data text width to find out the
     ...
     * data text always has same width, you can call this method tell to WheelSelector your data
     ...
     * {@link #setMaximumWidthTextPosition(int)} tell to WheelSelector where is the maximum width text
     * in data source, WheelSelector will calculate its width base on this text which found by
     * position. If you don't know the position of maximum width text in data source, but you have
     * maximum width text, you can call setMaximumWidthText tell to WheelSelector
     * what maximum width text is directly, WheelSelector will calculate its width base on this text.
     *
     ...
     * @see rygel.cn.dateselector.selector.WheelSelector.OnWheelChangeListener
     */
    void setOnWheelChangeListener(WheelSelector.OnWheelChangeListener listener);
     ...
     * <p>
     * Set whether WheelSelector display indicator or not
     * WheelSelector will draw two lines above an below current selected item if display indicator
     * Notice:Indicator's size will not participate in WheelSelector's size calculation, it will drawn
     * above the content
     *
     ...
     * 滚轮选择器是否有指示器
     * <p>
     * Whether WheelSelector display indicator or not
     *
     * @return 滚轮选择器是否有指示器
     */
     ...
     * <p>
     * Set whether WheelSelector display curtain or not
     * WheelSelector will draw a rectangle as big as current selected item and fill specify color
     ...
     * 滚轮选择器是否显示幕布
     * <p>
     * Whether WheelSelector display curtain or not
     *
     * @return 滚轮选择器是否显示幕布
     */
    boolean hasCurtain();
     ...
     * <p>
     * Set whether WheelSelector has atmospheric or not
     * WheelSelector's items will be transparent from center to ends if atmospheric display
     *
     ...
     * 滚轮选择器是否有空气感
     * <p>
     * Whether WheelSelector has atmospheric or not
     *
     * @return 滚轮选择器是否有空气感
     */
     ...
     * 滚轮选择器是否开启卷曲效果
     * <p>
     * Whether WheelSelector enable curved effect or not
     *
     ...
     * 的视觉差距
     * <p>
     * Set whether WheelSelector enable curved effect or not
     * If setCurved true, WheelSelector will display with curved effect looks like ends bend into
     * screen with perspective.
     * WheelSelector's curved effect base on strict geometric model, some parameters relate with size
     ...
     * 获取滚轮选择器数据项的对齐方式
     * <p>
     * Get alignment of WheelSelector
     *
    ...
    /**
     * 设置滚轮选择器数据项的对齐方式
     * 默认对齐方式为居中对齐{@link WheelSelector#ALIGN_CENTER}
     * <p>
     * Set alignment of WheelSelector
     * The default alignment of WheelSelector is {@link WheelSelector#ALIGN_CENTER}
     *
     * @param align 对齐方式标识值
     *              该值仅能是下列值之一：
     *              {@link WheelSelector#ALIGN_CENTER}
     *              {@link WheelSelector#ALIGN_LEFT}
     *              {@link WheelSelector#ALIGN_RIGHT}
     */
     ...
     * Set typeface of item text maybe cause WheelSelector size change
     *
     * @param tf 字体对象
     */
